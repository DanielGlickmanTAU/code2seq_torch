...
class TestMultiHeadAttention(TestCase):
    ...
    def test_shaping_position_attention_to_cross_graph_attention(self):
        T = 4
        B = 2
        N = 3
        num_heads = 3
        ...
        pos_attn_weights = PositionAttention.reshape_positional_attention_to_cross_graph_attention(pos_attn_weights, T)

        assert pos_attn_weights.shape == (B * num_heads, N, N * T)

        batch1_head1 = 0
        """pos cross attention should look like:
         <oo_1,oo1>  <oo_1,oo2> ..<oo_1,oo_T> <oo_1, oo1>, <oo_1,oo_2> ..<oo_1,oo_T> <ot_1,ot_1> <ot_1,ot_2> ..<ot_1,ot_t>
        """
        assert pos_attn_weights[batch1_head1][0][0] != pos_attn_weights[batch1_head1][0][1], \
            'expect <oo_l1,oo_l1> !=<oo_l1,oo_l2>'
